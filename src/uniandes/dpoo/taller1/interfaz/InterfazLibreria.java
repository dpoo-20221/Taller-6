...
public class InterfazLibreria extends JFrame
{
	...
	public void cargarArchivos(File archivo_categorias, File archivo_libros)
	{
		try
		{
			...
			HashMap<String, Integer> MapCategoriasNuevas = libreria.HashMapCatalogo();
			String textoCreado = "No se crearon nuevas Categorias";

			if (MapCategoriasNuevas.size() != 0 ){
				textoCreado = "Las categorias que se crearon fueron:";
				for (String categoria: MapCategoriasNuevas.keySet()) {
					String nombre = categoria;
					int tamanio = MapCategoriasNuevas.get(nombre);
	...
	{
		String nombreCategoria = JOptionPane.showInputDialog(this, "Escriba el nombre de la categoría que desea renombrar", "Categoría");
		String nuevoNombre = JOptionPane.showInputDialog(this, "Escriba el nombre Que le desea poner", "Renombrar");
		if (nombreCategoria != null && nuevoNombre != null)
		{
			Categoria categoria = libreria.buscarCategoria(nombreCategoria);
			if (categoria == null)
			{
				JOptionPane.showMessageDialog(this, "No hay ninguna categoría con ese nombre", "No hay categoría",
						JOptionPane.INFORMATION_MESSAGE);
			}
			else
			{
				try 
				{
					String mensaje = "Se realizó correctamente el renombre:\n";
	...
	{
		String autores = JOptionPane.showInputDialog(this, "Escriba el nombre o los nombres de los autores separados por ,");
		if (autores != null)
		{
		  try 
			{
				String mensaje = "Se eliminaron correctamente los libros de los autores dados:\n";
				libreria.borrarLibros(autores);
				JOptionPane.showMessageDialog(this, mensaje, "Autores", JOptionPane.INFORMATION_MESSAGE);
			}
	      catch(Exception e)
			{
					JOptionPane.showMessageDialog(this, e.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
		...
		{
			JOptionPane.showMessageDialog(this, "Porfavor ingrese al menos un nombre de un autor", "No hay autores",
					JOptionPane.INFORMATION_MESSAGE);
		}
